...
@Test func pulledSanFranciscoEvidenceHashesMatchCatalog() throws {
    ...
    #expect(SHA256.hex(grooveData) == groove.sha256)
    ...
    let fogURL = root.appendingPathComponent(fog.source!)
    let fogData = try Data(contentsOf: fogURL)
    #expect(SHA256.hex(fogData) == fog.sha256)
